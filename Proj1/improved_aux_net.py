...
def train_AuxNet(model, eta, train_loader, test_loader, eval_mode=False, optim = 'Adam'):
    losses = []
    train_error_rates = []
    test_error_rates = []
    device = get_device() 
...
def binary_search_AuxNet(hidden_layers1, hidden_layers2, hidden_layers3, batch_sizes, etas, dropout_probabilities, optim):
                        ...
                        for eta in etas:
                            for do in dropout_probabilities:
                                ...
                                for r in range(NUMBER_OF_SEARCH_RUNS):
                                    ...
                                    _, _, er = train_AuxNet(model, eta, train_loader, test_loader, optim)
                                    error_rate += er[-1]
                                    del model
...
def eval_AuxNet(optim):
    ...
    hl, h2, h3, bs, eta, do = binary_search_AuxNet(hidden_layers1, hidden_layers2, hidden_layers3, batch_sizes, etas, dropout_probabilities, optim)
    ...
    for i in range(NUMBER_OF_EVALUATION_RUNS):
        model = AuxNet(hl, h2, h3, do)
        train_loader, test_loader = get_data(N=1000, batch_size=2**bs, shuffle=True)
        losses, train_error_rates, test_error_rates = train_AuxNet(model, eta, train_loader, test_loader, optim)
...
def run_AuxNet(hl, h2, h3, bs, eta, do, optim):
    averaged_time = 0
    averaged_losses = 0
    averaged_train_error_rate = 0
    averaged_test_error_rate = 0
